...
export const driverRepository = {
  ...
  async findDriversForUser(userId: string, userRole: string) {
    ...
    return prisma.driver.findMany({
      where,
      include: {
        user: { select: { id: true, name: true, email: true, phone: true, avatar: true, department: true } },
        vehicleAssignments: {
          where: { isActive: true },
          include: {
    ...
    return prisma.driver.findUnique({
      ...
      include: {
        ...
        vehicleAssignments: {
          where: { isActive: true },
          include: {
            vehicle: { select: { id: true, plate: true, model: true, capacity: true, status: true } },
          },
        },
      },
    })
   ...
   */
  async isLicenseNumberUnique(licenseNumber: string, excludeId?: string): Promise<boolean> {
  ...
  async getDriverStats(userRole: string, userId?: string) {
    ...
    const [totalDrivers, availableDrivers, onTripDrivers, offDutyDrivers] = await Promise.all([
