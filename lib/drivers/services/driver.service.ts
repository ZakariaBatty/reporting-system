...
export const driverService = {
  ...
  async getDriver(driverId: string, userId: string, userRole: string) {
    ...
    // Check authorization
    const canViewAllDrivers = ['manager', 'admin', 'super_admin'].includes(userRole)
   ...
   */
  async updateDriver(driverId: string, userId: string, userRole: string, driverData: DriverUpdateData) {
    // Check authorization
    const canUpdateAllDrivers = ['manager', 'admin', 'super_admin'].includes(userRole)
    ...
    if (driverData.licenseNumber && driverData.licenseNumber !== driver.licenseNumber) {
      const isUnique = await driverRepository.isLicenseNumberUnique(driverData.licenseNumber, driverId)
      if (!isUnique) {
    ...
    if (driverData.licenseNumber) updateInput.licenseNumber = driverData.licenseNumber
    if (driverData.licenseExpiry) updateInput.licenseExpiry = new Date(driverData.licenseExpiry)
    if (driverData.totalTrips !== undefined) updateInput.totalTrips = driverData.totalTrips
    if (driverData.totalKm !== undefined) updateInput.totalKm = driverData.totalKm
    if (driverData.averageRating !== undefined) updateInput.averageRating = driverData.averageRating
   ...
   */
  async canUserAccessDriver(driverId: string, userId: string, userRole: string): Promise<boolean> {
