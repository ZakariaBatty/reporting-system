...
export const tripService = {
  ...
  async getTripStats(userId: string, userRole: string) {
    return tripRepository.getTripStats(userRole, userRole === 'driver' ? userId : undefined)
  },
  ...
  async updateTrip(
    ...
    // Check authorization
    const canUpdateAllTrips = ['manager', 'admin', 'super_admin'].includes(userRole)
    ...
    if (tripData.departureTime) updateInput.departureTime = tripData.departureTime
    ...
    if (tripData.pickupLocation) updateInput.pickupLocation = tripData.pickupLocation
    if (tripData.dropoffLocation) updateInput.dropoffLocation = tripData.dropoffLocation
    ...
    if (tripData.passengersCount) updateInput.passengersCount = tripData.passengersCount
    ...
    if (tripData.tripPrice !== undefined) updateInput.tripPrice = tripData.tripPrice
    if (tripData.actualCost !== undefined) updateInput.actualCost = tripData.actualCost
    ...
    if (canUpdateAllTrips) {
      ...
      if (tripData.hotelId) updateInput.hotel = { connect: { id: tripData.hotelId } }
      if (tripData.vehicleId)
        updateInput.vehicle = { connect: { id: tripData.vehicleId } }
      if (tripData.driverId) updateInput.driver = { connect: { id: tripData.driverId } }
    }
