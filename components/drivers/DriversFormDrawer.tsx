...
export function DriversFormDrawer({ isOpen, mode, driver, onClose, onSave, isSaving = false }: DriversFormDrawerProps) {
      ...
      >
        <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">
            {mode === 'create' ? 'Add Driver' : mode === 'edit' ? 'Edit Driver' : 'Driver Details'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {mode === 'view' && formData ? (
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 mt-1">{formData.user?.name || 'N/A'}</p>
              </div>

              <div>
                <label className="text-sm font-medium text-gray-600">Email</label>
                <p className="text-gray-900 mt-1">{formData.user?.email || 'N/A'}</p>
              </div>

              <div>
                <label className="text-sm font-medium text-gray-600">Phone</label>
                <p className="text-gray-900 mt-1">{formData.user?.phone || 'N/A'}</p>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-900 mt-1">{formatDate(formData.licenseExpiry)}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">{formData.totalTrips}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600">Total KM</label>
                  <p className="text-2xl font-bold text-gray-900 mt-1">{formData.totalKm}</p>
                </div>
              </div>

              <div>
                <label className="text-sm font-medium text-gray-600">Average Rating</label>
                <p className="text-2xl font-bold text-yellow-500 mt-1">{formData.averageRating.toFixed(1)}</p>
              </div>
            </div>
          ) : (
            // Edit/Create Mode
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData!, totalTrips: parseInt(e.target.value) })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  disabled={mode === 'view'}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData!, totalKm: parseInt(e.target.value) })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  disabled={mode === 'view'}
                />
              </div>
          ...
          {mode !== 'view' && (
            <div className="flex gap-3 pt-6 border-t border-gray-200">
              ...
              >
                {isSaving ? 'Saving...' : mode === 'create' ? 'Add Driver' : 'Save Changes'}
              </button>
            </div>
          )}
