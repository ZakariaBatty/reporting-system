...
import { getDrivers, updateDriver, deleteDriver, getDriverStats } from '@/app/drivers/actions'
...
export function DriversContainer() {
  ...
  const loadDrivers = useCallback(async () => {
    try {
      setIsLoading(true)
      const [driversData, statsData] = await Promise.all([getDrivers(), getDriverStats()])
        ...
        setStats((prev) =>
          prev
            ? {
                ...
                ...(driver.status === 'AVAILABLE' && { availableDrivers: prev.availableDrivers - 1 }),
                ...(driver.status === 'ON_TRIP' && { onTripDrivers: prev.onTripDrivers - 1 }),
                ...(driver.status === 'OFF_DUTY' && { offDutyDrivers: prev.offDutyDrivers - 1 }),
              }
            : null,
        )
      } catch (error) {
