...
export function DriversList({
  ...
  if (isLoading) {
    return <div className="text-center py-12 text-gray-500">Loading drivers...</div>
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {drivers.map((driver) => (
        <div key={driver.id} className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-lg transition-shadow">
          {/* Header */}
          <div className="flex items-start justify-between mb-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">{driver.user?.name || 'Unknown'}</h3>
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className="text-sm text-gray-600 ml-1">{driver.averageRating.toFixed(1)}</span>
              </div>
            </div>
            <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(driver.status)}`}>
          ...
          <div className="space-y-2 mb-4 pb-4 border-b border-gray-200 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-900">{formatDate(driver.licenseExpiry)}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">{driver.totalKm}</p>
              <p className="text-xs text-gray-600">km</p>
            </div>
            <div className="text-center">
              <p className="text-lg font-bold text-purple-600">{driver.vehicleAssignments?.length || 0}</p>
