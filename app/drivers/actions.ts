...
export async function updateDriver(
  ...
  try {
    ...
    const userRole = (session.user as any).role || 'driver'
    return await driverService.updateDriver(driverId, session.user.id, userRole, data)
  } catch (error: any) {
...
export async function getDriverStats() {
  try {
    ...
    const { driverRepository } = await import('@/lib/drivers/repositories/driver.repository')
